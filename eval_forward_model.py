...
root_path = "forawrd_model/datasets/DTU24"
vis_out_dir = "forawrd_model/vis_out/val"
weight_save_dir = "forawrd_model/weights"
dataset = forward_model_dataset(root_path)

read_pth_exp = "20250729210705"

read_pth_exp_path = os.path.join(weight_save_dir,read_pth_exp)

# for pth_name in os.listdir(read_pth_exp_path):
pth_files  =  os.listdir(read_pth_exp_path)
pth_files.sort(key=lambda x: int(x.split(".")[0].rsplit("_",1)[-1]), reverse=True)

newest_pth = os.path.join(read_pth_exp_path,pth_files[0])
# model = torch.load(newest_pth, map_location=torch.device(device)).to(device)
model.load_state_dict(torch.load(newest_pth, map_location=torch.device(device)))
...
exp_vis_out_dir = os.path.join(vis_out_dir,read_pth_exp)
# exp_weight_save_dir = os.path.join(weight_save_dir,str(datetime_num))
if not os.path.exists(exp_vis_out_dir):
...
# img1,img2 = cv2.imread("DTU/set_23_24_33/scan24/images/0000.png"), cv2.imread("DTU/set_23_24_33/scan24/images/0001.png")
	...
	tcnn_pred = model( torch.cat([reshaped_pcd1, reshaped_color1], dim=1))
...
# if i%100==0:
	...
	cv2.imwrite(os.path.join(exp_vis_out_dir,f"render_{i}.png"),img_vis)
	
	# torch.save(model, os.path.join(exp_weight_save_dir,f"model_weight_{i}.pth"))
	...
	PlyData([el]).write(os.path.join(exp_vis_out_dir,f"GS_{i}.ply"))
	print("ply saved")
