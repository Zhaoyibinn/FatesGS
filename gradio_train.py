...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint,args):
    ...
    # yield scene.gaussians.get_xyz.cpu().detach().numpy()
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        total_loss_diff = 0
        if int(viewpoint_cam.image_name) == 0:
            if iteration % 100 <3:
                ...
                point_cloud_data = trimesh.PointCloud(vertices=np.array(xyz),colors=np.zeros_like(np.array(xyz)))
                scene_3d.add_geometry(point_cloud_data)
                scene_3d.export(file_obj="test.glb")
                yield str(iteration) + " / "+ str(opt.iterations),np.uint8(image.cpu().detach().numpy().transpose(1,2,0)* 255),"test.glb",args.model_path,args.source_path
...
def gradio_train_input(input_images):
    ...
    # img_input_list = []
    # for i in ["DTU/wo_pose/scan24/images/0000.png","DTU/wo_pose/scan24/images/0001.png","DTU/wo_pose/scan24/images/0002.png"]:
    #     img_input_list.append(cv2.imread(i))

    # input_root_dir = "DTU/wo_pose/scan24"
    
    args = parser.parse_args(sys.argv[1:])
    # lp.use_diff = args.diff
    args.save_iterations.append(args.iterations)
    
    gradio_huancun_path = "gradio_huancun"
    time = datetime.now().strftime("%m_%d_%Y_%H:%M:%S")
    input_root_dir = os.path.join(gradio_huancun_path, time,"input")
    output_root_dir = os.path.join(gradio_huancun_path, time,"output")
    os.mkdir(os.path.join(gradio_huancun_path, time))
    os.mkdir(os.path.join(gradio_huancun_path, time,"input"))
    os.mkdir(os.path.join(gradio_huancun_path, time,"input","images"))
    os.mkdir(os.path.join(gradio_huancun_path, time,"output"))

    img_names = ["0000.png","0001.png","0002.png"]

    for idx,img_gradio_path in enumerate(input_images):
        img_name = img_gradio_path.split("/")[-1]

        img = cv2.imread(img_gradio_path)
        ...
        cv2.imwrite(os.path.join(input_root_dir,"images",img_names[idx]),img)
        # shutil.copy(img_gradio_path,os.path.join(input_root_dir,"images",img_names[idx]))

    shutil.copy("DTU/origin/set_23_24_33/scan24/pair.txt",os.path.join(input_root_dir,"pair.txt"))





    args.source_path = input_root_dir
    args.model_path = output_root_dir
    args.resolution = 2
    args.vggt = True
    ...
    yield from training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint,args)
