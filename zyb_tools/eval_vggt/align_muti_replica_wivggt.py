...
parser.add_argument('--vggt_path', type=str, required=True, help='输入根目录')
...
# for scan in [24]:
    ...
    input_path_root_vggt = args.vggt_path
    output_path_root = input_path_root

    # input_path = os.path.join(input_path_root,f"scan{scan}/train/ours_{iteration}/fuse.ply")

    input_path = os.path.join(input_path_root,f"{scan}/train/ours_{iteration}/fuse_post.ply")
    input_path_2 = os.path.join(input_path_root,f"{scan}/train/ours_1/fuse.ply")
    input_path_vggt = os.path.join(input_path_root_vggt,f"{scan}/train/ours_1/fuse.ply")
    output_path = os.path.join(output_path_root,f"{scan}/train/ours_{iteration}/fuse_aligned.ply")
    output_path_2 = os.path.join(output_path_root,f"{scan}/train/ours_1/fuse_aligned.ply")
    output_path_vggt = os.path.join(output_path_root,f"{scan}/train/ours_{iteration}/fuse_aligned_vggt.ply")
    ...
    input_ply_posealign_numpy_2 = trans_with_rst(np.array(input_ply_2.points), np.array(s), np.array(R), np.array(t))
    input_ply_posealign_numpy_vggt = trans_with_rst(np.array(input_ply_vggt.points),np.array(s),np.array(R),np.array(t))
    ...
    T = align1_reg(input_ply_posealign_numpy,np.array(depth_gt_ply.points))
    R = T[:3,:3]
    t = T[:3,3]

    input_ply_reg_numpy = trans_with_rt(input_ply_posealign_numpy,R,t)
    input_ply_reg_numpy_2 = trans_with_rt(input_ply_posealign_numpy_2,R,t)
    input_ply_reg_numpy_vggt = trans_with_rt(input_ply_posealign_numpy_vggt,R,t)
    ...
    # input_ply_regmasked_numpy = mask_dtu(scan,input_ply_rescale_numpy)
    # input_ply_regmasked_numpy = input_ply_rescale_numpy
    ...
    o3d.io.write_point_cloud(output_path,o3d.geometry.PointCloud(o3d.utility.Vector3dVector(input_ply_reg_numpy)),write_ascii=True)
    o3d.io.write_point_cloud(output_path_2,o3d.geometry.PointCloud(o3d.utility.Vector3dVector(input_ply_reg_numpy_2)),write_ascii=True)
    o3d.io.write_point_cloud(output_path_vggt,o3d.geometry.PointCloud(o3d.utility.Vector3dVector(input_ply_reg_numpy_vggt)),write_ascii=True)
    ...
    eval_cd_savedir_root = os.path.join(output_path_root,f"{scan}/train")
    # eval_cd_savedir_vggt = os.path.join(eval_cd_savedir_root,"eval_vggt")
    eval_cd_savedir = os.path.join(eval_cd_savedir_root,f"eval_ours_{iteration}")
    eval_cd_savedir_2 = os.path.join(eval_cd_savedir_root,f"eval_ours_1")
    eval_cd_savedir_vggt = os.path.join(eval_cd_savedir_root,f"eval_vggt_{iteration}")
    ...
    os.system(f"python zyb_tools/eval_replica/eval.py --data {output_path}  --mode pcd --dataset_dir Replica/replica_gt/{scan} --vis_out_dir {eval_cd_savedir}")
    os.system(f"python zyb_tools/eval_replica/eval.py --data {output_path_2}  --mode pcd --dataset_dir Replica/replica_gt/{scan} --vis_out_dir {eval_cd_savedir_2}")
    os.system(f"python zyb_tools/eval_replica/eval.py --data {output_path_vggt}  --mode pcd --dataset_dir Replica/replica_gt/{scan} --vis_out_dir {eval_cd_savedir_vggt}")
